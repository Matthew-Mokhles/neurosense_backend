...
model: XGBoostRandomForestModel | None = None
...
app = FastAPI(
    ...
    description=(
        "Q-CHAT-10 autism screening for toddlers using an XGBoost + Random Forest ensemble model. "
        "POST /classify expects each A1–A10 as a paper column letter A–E (case-insensitive). "
        "Official scoring: items 1–9 count 1 for C, D, or E; item 10 counts 1 for A, B, or C."
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
@app.post("/classify")
def classify(answers: ScreeningAnswers):
